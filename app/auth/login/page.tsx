    ...
    try {
      if (isSignUp) {
        const { data, error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: 'https://finanzplan-seven.vercel.app/auth/callback',
          },
        })
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm">
        <div className="mb-10 text-center">
          <h1 className="text-4xl mb-2" style={{ fontFamily: 'var(--font-display)' }}>Finanzplan</h1>
          <p className="text-muted-foreground text-sm">Persönliche Budgetplanung</p>
        </div>
          ...
          {step === 'login' ? (
            <>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                    ...
                    placeholder="name@beispiel.ch" required autoFocus autoComplete="email" />
                </div>
                <div>
                  ...
                  {isSignUp && <p className="text-xs text-muted-foreground mt-1">Mindestens 8 Zeichen</p>}
                </div>
                {isSignUp && (
                  <div>
                    <label className="block text-sm text-muted-foreground mb-1.5">Passwort bestätigen</label>
                    <input type="password" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)}
                      className={`w-full bg-background border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 transition-colors ${
                        passwordConfirm && password !== passwordConfirm ? 'border-red-400' : 'border-border focus:border-primary'
                      }`}
                ...
                {message && <p className="text-sm text-green-600 bg-green-50 dark:bg-green-950/30 rounded-lg px-3 py-2">{message}</p>}
                <button type="submit" disabled={loading || (isSignUp && password !== passwordConfirm)}
              ...
              <div className="mt-6 pt-5 border-t border-border text-center">
                <button onClick={() => { setIsSignUp(!isSignUp); setError(''); setMessage(''); setPasswordConfirm('') }}
            ...
            <>
              <h2 className="text-lg font-medium mb-2" style={{ fontFamily: 'var(--font-display)' }}>Zwei-Faktor-Code</h2>
              <p className="text-sm text-muted-foreground mb-6">Gib den 6-stelligen Code aus deiner Authenticator-App ein.</p>
